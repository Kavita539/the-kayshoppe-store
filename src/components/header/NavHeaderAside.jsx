...
const NavHeaderAside = ({ navAside, setNavAside }) => {
const {
...
return (
<>
  <div className={`nav-aside-background ${navAside ? "show-nav-background" : "" }`} onClick={()=> setNavAside(prevState => !prevState)}></div>
    ...
    <div className="nav-aside-head">
      <div>
        ...
        <img className="responsive-img rounded-img"
          src={`https://avatars.dicebear.com/api/identicon/your-custom-seed-${userName}.svg`} alt="avatar" />
        </Link>
        <span>{userName}</span>
      </div>
